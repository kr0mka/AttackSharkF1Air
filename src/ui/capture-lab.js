...
export function renderCaptureLab(state, connected) {
  ...
  return `<div class="card capture-lab" id="capture-lab">
    <div class="card-head"><div><h3>Protocol capture lab</h3><p>Read a baseline, change exactly one setting in Control HUB, then read the same region and compare.</p></div><span class="tag blue">Read only</span></div>
    ...
    <div class="grid two">
      ...
      <div class="control"><label for="capture-notes">Single setting changed / official values</label><input id="capture-notes" type="text" maxlength="2000" placeholder="Example: Dynamic Sensitivity Classic → Natural" value="${esc(state.notes)}"></div>
    </div>
